// src/pages/FossilMintNaturalStoneView.jsx
import React, { useRef, useEffect, useState, useCallback } from "react";
import gsap from "gsap";
import { ScrollTrigger } from "gsap/ScrollTrigger";
import { Link } from "react-router-dom";
import Footer from "@/components/Footer";
import { ArrowRight, PhoneCall, ShieldCheck, Gem, Layers, ChevronRight, CheckCircle2, ZoomIn, Globe, Sun, PenTool, LayoutGrid } from "lucide-react";
import { stoneProductsData } from "../data/stoneProductsData";

gsap.registerPlugin(ScrollTrigger);

const HERO_IMG = "/img/product/Stone Products/Gwalior Stone/Fossil Mint Natural stone/Fossil-mint-natural-stone-1.webp";
const MARBLE_TEXTURE = "/img/product/Stone Products/Gwalior Stone/Fossil Mint Natural stone/pattern 2.webp";
...
const features = [
  { name: "Timeless Charm", icon: Gem, desc: "Add timeless charm to any area with unique patterns and tonal variations.", color: "#A9C4A0", img: "/img/product/Stone Products/Gwalior Stone/Fossil Mint Natural stone/pattern 1.webp" },
  { name: "Natural Textures", icon: Sun, desc: "Natural colors and textures creating off-white cream to yellow shades.", color: "#4A6D7C", img: "/img/product/Stone Products/Gwalior Stone/Fossil Mint Natural stone/pattern 2.webp" },
  { name: "Unique Quality", icon: ShieldCheck, desc: "Unique quality that is hard to wear preventing the loss of color or surface.", color: "#C9A66B", img: "/img/product/Stone Products/Gwalior Stone/Fossil Mint Natural stone/pattern 3.webp" },
  { name: "Hand-cut Edges", icon: PenTool, desc: "Split surface and hand-cut dressed edges for an authentic natural look.", color: "#8B8D8A", img: "/img/product/Stone Products/Gwalior Stone/Fossil Mint Natural stone/Fossil-mint-natural-stone-2.webp" },
];

const galleryImages = [
  "/img/product/Stone Products/Gwalior Stone/Fossil Mint Natural stone/Fossil-mint-natural-stone-1.webp",
  "/img/product/Stone Products/Gwalior Stone/Fossil Mint Natural stone/pattern 1.webp",
  "/img/product/Stone Products/Gwalior Stone/Fossil Mint Natural stone/Fossil-mint-natural-stone-2.webp",
  "/img/product/Stone Products/Gwalior Stone/Fossil Mint Natural stone/pattern 2.webp",
  "/img/product/Stone Products/Gwalior Stone/Fossil Mint Natural stone/Fossil-mint-natural-stone-3.webp",
  "/img/product/Stone Products/Gwalior Stone/Fossil Mint Natural stone/pattern 3.webp",
];

const steps = [
  { title: "Stone Sourcing", body: "We source the highest quality Mint Fossil Sandstone, known for its flawless range and durability." },
  { title: "Precision Hand-Cutting", body: "Expert professionals cut the stones to create dressed edges and appealing shapes." },
  { title: "Surface Finishing", body: "Available with lightly riven surfaces, honed, sawn, or sandblasted finishes for aesthetic appeal." },
  { title: "Quality Control", body: "Every stone is checked to ensure it meets international standards for interiors and exteriors." },
];

const LENS_SIZE = 180;
const ZOOM = 2.4;

const FossilMintNaturalStoneView = () => {
  ...
  return (
    <div ref={pageRef} className="bg-[#FAFAF8] min-h-[100svh] font-sans text-[#111]">
      ...
      <section className="relative h-[100svh] min-h-[500px] sm:min-h-[600px] w-full overflow-hidden bg-[#111]">
        ...
        <div className="relative z-10 h-full flex flex-col items-center justify-end sm:justify-center text-center px-4 sm:px-6 pb-20 sm:pb-0 max-w-5xl mx-auto">
          <div ref={heroBadgeRef} className="opacity-0 inline-flex items-center gap-2 px-5 py-2 rounded-full border border-white/20 bg-white/5 backdrop-blur-md mb-8">
            ...
            <span className="text-xs tracking-[0.2em] text-[#DFBA73] font-bold uppercase">Premium Collection</span>
          </div>
          ...
          <p ref={heroSubRef} className="opacity-0 text-sm sm:text-lg md:text-2xl text-white/80 font-light max-w-2xl mx-auto leading-relaxed mb-6 sm:mb-10">
            A lightly riven surface giving a combination of colors with natural marks and vibrant tones.
          </p>
          ...
          <div ref={heroCtaRef} className="opacity-0 flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-6 w-full sm:w-auto">
            ...
            <a href="#features" className="px-6 sm:px-8 py-3 sm:py-4 rounded-full border border-white/30 text-white font-medium text-xs sm:text-sm hover:bg-white hover:text-black transition-colors backdrop-blur-sm flex items-center gap-2">
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-14 lg:gap-20 items-center">
          <div className="split-left space-y-6">
            ...
            <p className="text-[15px] text-[#666] leading-[1.85] font-light">
              Fossil Mint Natural Stone is our most popular stone product with split surface and dressed edges that come at an economical price. We are leading Fossil Mint Natural Stone Manufacturers in India. With its lightly riven surface, it gives a combination of colors with natural marks and vibrant tones making it the best natural mint stone in India.
            </p>
            <p className="text-[15px] text-[#666] leading-[1.85] font-light">
              Fossil Mint Natural Stone is hard to wear preventing the loss of color or surface even after several years of use. Paving displays predominant tones create an off-white cream to off-white yellow shades with unique patterns, tonal variations, and nominal differences in color to provide a unique charm. Natural mint stones have major applications in exterior areas such as in garden patios as well as interior stone floors.
            </p>
            <p className="text-[15px] text-[#666] leading-[1.85] font-light">
              Mint natural stone gives an appealing look on both smaller and larger areas. It is aesthetically fit for laying paving where depth restrictions occur, it enables a more accurate level working to make laying simpler. It comes in various finishes such as polished, honed, sawn, sandblasted, etc.
            </p>
            <div className="pt-4">
        ...
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-14 lg:gap-20 items-center reveal-block">
          <div className="split-left">
            ...
            <p className="text-[15px] text-[#666] leading-[1.85] font-light mb-5">
              Hover over the stone to inspect the unique patterns, tonal variations, and nominal differences in color that provide a unique charm.
            </p>
      ...
      {/* 2.2. FEATURES / PATTERNS */}
      <section id="features" className="reveal-block py-20 sm:py-28 px-6 bg-[#FAFAF8]">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-14">
            <p className="text-[11px] tracking-[0.25em] font-bold text-[#B8955D] uppercase mb-4 font-mono">Unmatched Quality</p>
            ...
            <p className="text-[15px] text-[#666] leading-[1.85] font-light max-w-2xl mx-auto mt-4">
              These stones are appreciated for their significant features like longer service life, durability and economical price. Our experienced professionals process the high-quality Mint Fossil Natural Sandstone to bring appealing shape and texture.
            </p>
          </div>

          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-center">
            <div className="lg:col-span-5 flex flex-col gap-3 pb-2 lg:pb-0">
              {features.map((f, i) => (
                <button
                  key={f.name}
                  onClick={() => setActiveFeature(i)}
                  className={`flex items-center gap-4 px-5 py-4 rounded-2xl border text-left transition-all duration-300 ${
                    i === activeFeature ? "border-[#B8955D] bg-[#B8955D]/5 shadow-md scale-105" : "border-[#EDEDE9] hover:border-[#B8955D]/40"
                  }`}
                >
                  <div className="w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0" style={{ backgroundColor: f.color + "20", color: f.color }}>
                    <f.icon size={20} />
                  </div>
                  <div>
                    <p className="font-serif font-bold text-[#111]">{f.name}</p>
                  </div>
                </button>
              ))}
            </div>
              ...
              <div className="relative aspect-[16/10] rounded-3xl overflow-hidden shadow-2xl mb-6">
                {features.map((f, i) => (
                  <img
                    ...
                    style={{ opacity: i === activeFeature ? 1 : 0 }}
                  />
                ))}
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent pointer-events-none" />
                <div className="absolute bottom-8 left-8 right-8 pointer-events-none text-left">
                  <h3 className="text-3xl font-serif text-white mb-3">{features[activeFeature].name}</h3>
                  <p className="text-white/90 text-lg leading-relaxed max-w-xl font-light">{features[activeFeature].desc}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="max-w-[90rem] mx-auto px-6 md:px-12">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 border-t border-black/[0.08]">
            {stoneProductsData.grid
              .filter(item => item.title !== "Fossil Mint Natural Stone")
              .slice(0, 8)
              .map((item, idx) => (
              <Link
                key={idx}
      ...
      <section className="reveal-block py-20 sm:py-28 px-6 pb-32">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-14">
            ...
            <h2 className="text-3xl sm:text-4xl font-serif text-[#111] leading-[1.2]">
              From Quarry to <span className="text-[#B8955D] italic">Finished Stone</span>
            </h2>
          </div>
      ...
      <section id="contact" className="relative py-32 flex items-center justify-center overflow-hidden bg-[#111]">
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-6 text-center flex flex-col items-center">
          ...
          <p className="text-lg md:text-xl text-white/70 font-light max-w-2xl mx-auto mb-10 leading-relaxed">
            Available in bulk for wholesale buyers globally — the perfect choice for garden patios,
            pathways, and elegant interior stone floors.
          </p>
...
export default FossilMintNaturalStoneView;
