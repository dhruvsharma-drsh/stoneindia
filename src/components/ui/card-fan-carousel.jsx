...
export default function SocialCards({ cards }) {
      ...
      visibleEntries.forEach(({ el, slot }) => {
        ...
        if (hoveredSlot !== null) {
          ...
          if (slot === hoveredSlot) {
            ...
            targetScale *= 1.08;
            if (slot === 0) {
              targetX -= 6 * mult; // move outward left (subtle)
              targetRot -= 3;
            } else if (slot === visibleEntries.length - 1) {
              targetX += 6 * mult; // move outward right (subtle)
              targetRot += 3;
            }
          } else {
  ...
  const chevron = (direction) => (
    <svg className="relative z-[2] w-4 h-4 md:w-5 md:h-5 text-black/60" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
  ...
  return (
    <section className="flex flex-col items-center w-full py-4 lg:py-8 px-4 md:px-8 relative z-20">
      <div className="flex items-center justify-center w-full max-w-[90rem]">
        <div ref={containerRef} className="fan-layout flex relative justify-center items-center w-full max-w-[80rem] h-[400px] sm:h-[500px] lg:h-[600px]">
          {cards.map((card, index) => {
            const image = (
              <div className="relative w-full h-full overflow-hidden rounded-[2rem] shadow-[0_10px_30px_rgba(0,0,0,0.12)] border border-black/5 bg-white">
                <img src={card.imgUrl} loading="lazy" alt={card.alt || `Card ${index}`} className="absolute inset-0 w-full h-full object-cover z-10" />
                <div className="absolute inset-x-0 bottom-0 h-[60%] bg-gradient-to-t from-white via-white/80 via-40% to-transparent z-20 pointer-events-none" />
                <div className="absolute bottom-6 left-6 right-6 z-30 text-left pointer-events-none">
                  <h3 className="text-[#111] font-sans text-xl sm:text-2xl mb-1 drop-shadow-sm">{card.title}</h3>
                  <p className="text-[#666] font-sans text-xs sm:text-sm line-clamp-2">{card.desc}</p>
                </div>
              </div>
            );
            return card.linkUrl ? (
              <Link key={index} to={card.linkUrl} className="fan-card block cursor-pointer absolute w-[240px] sm:w-[280px] lg:w-[320px] aspect-[3/4] hover:z-50">{image}</Link>
            ) : (
              <div key={index} className="fan-card absolute w-[240px] sm:w-[280px] lg:w-[320px] aspect-[3/4]">{image}</div>
            );
          })}
        </div>
      </div>
